...
class ComposeImporter:
    ...
    def parse_compose_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"Parsed compose file with {len(content.get('services', {}))} services")
            return content
    ...
    def _convert_single_service(self, name: str, config: Dict, networks: Dict, volumes: Dict) -> Dict:
    ...
    def import_from_git(self, repo_url: str, branch: str = 'main',
                       compose_file_path: str = None) -> Tuple[List[Dict], Dict]:
        """
    ...
    def validate_service_for_swarm(self, service: Dict) -> List[str]:
        ...
        if service.get('build_context'):
            warnings.append("Build contexts are not supported in Swarm mode. Pre-built images are required.")

        if service.get('depends_on'):
            warnings.append("depends_on is not directly supported in Swarm mode. Consider using healthchecks.")

        # Check resource limits format
        resources = service.get('resources', {})
        if resources.get('memory_limit'):
            memory = resources['memory_limit']
            if isinstance(memory, str) and not any(unit in memory.lower() for unit in ['k', 'm', 'g']):
                warnings.append("Memory limit should include units (e.g., '512m', '1g')")
