        ...
        if options.get('service'):
            ...
            try:
                if docker_manager.client:
                    ...
                    if service:
                        self.stdout.write(f"Found service: {service.name} (ID: {service.id})")
                        
                        # Test logs
                        logs_data = docker_manager.get_service_logs(service.id, lines=10)
                        
                        self.stdout.write(f"Logs result: {len(logs_data.get('logs', []))} lines")
                        if logs_data.get('error'):
                            self.stdout.write(self.style.ERROR(f"Error: {logs_data['error']}"))
